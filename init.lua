...
require('lazy').setup({
  ...
  { -- Useful plugin to show you pending keybinds.
    ...
    opts = {
      ...
      spec = {
        ...
        { '<leader>r', group = '[R]eplace in file', mode = 'v' },
        { '<leader>s', group = '[S]earch' },
        { '<leader>w', group = '[W]orkspace' },
        { '<leader>t', group = '[T]oggle' },
        { '<leader>h', group = 'Git [H]unk', mode = { 'n', 'v' } },
        { '<leader>g', group = '[G]it link', mode = { 'n', 'v' } },
      },
    },
  },
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sG', function()
        local args = vim.deepcopy(config.vimgrep_arguments)
        table.insert(args, '--hidden') -- include hidden files
        builtin.live_grep {
  ...
  },
  {
    ...
    config = function()
      ...
      vim.diagnostic.config {
        ...
        -- Open the float on `[d` and `]d` so a jump also shows what it landed on.
        jump = {
      ...
      -- mason-lspconfig v2 dropped `handlers`, and it merges unknown keys
      -- silently, so a stale `handlers` table fails without any error. Server
      -- config now goes through `vim.lsp.config`, which layers onto the
      ...
      require('mason-lspconfig').setup {
        -- v2 enables every installed package that maps to an LSP server, which
        -- catches stylua: nvim-lspconfig runs it as `stylua --lsp`, a flag this
        -- build rejects. Conform already formats Lua with it, so it stays a
        -- formatter rather than becoming a failing client on every startup.
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function()
      ...
      --  to run regex highlighting alongside treesitter and skip its indentexpr.
      local needs_regex_highlight = { ruby = true }
      ...
      local function try_attach(buf, language)
        ...
        if needs_regex_highlight[language] then
          vim.bo[buf].syntax = 'on'
          return
        end

        -- Without an indent query the indentexpr would break indenting outright,
        -- so leave vim's built-in one in place when there isn't one.
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(args)
          ...
          elseif vim.tbl_contains(available, language) then
            -- Replaces the old `auto_install`: fetch on first sighting, attach after
